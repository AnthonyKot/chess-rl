...
interface Layer {
    ...
    /**
     * Optional hook allowing layers to expose the L2 norm of their accumulated gradients.
     * Implementations can return null when gradient information is not available.
     */
     fun gradientL2Norm(): Double? = null
}
...
class DenseLayer(
    ...
    override fun gradientL2Norm(): Double? {
        if (gradientCount == 0) return null
        var sumSquares = 0.0
        val invCount = 1.0 / gradientCount
        for (i in 0 until outputSize) {
            for (j in 0 until inputSize) {
                val grad = weightGradients[i][j] * invCount
                sumSquares += grad * grad
            }
        }
        for (i in 0 until outputSize) {
            val grad = biasGradients[i] * invCount
            sumSquares += grad * grad
        }
...
class FeedforwardNetwork(
    ...
    companion object {
        private const val DEFAULT_GRADIENT_NORM = 1.0
    }
     ...
     */
    fun trainBatch(batch: TrainingBatch, epoch: Int = 0, captureGradientNorm: Boolean = false): TrainingMetrics {
        var totalLoss = 0.0
        ...
        val measuredGradientNorm = if (captureGradientNorm) {
            measureGradientNorm() ?: DEFAULT_GRADIENT_NORM
        } else {
            DEFAULT_GRADIENT_NORM
        }
        ...
        return TrainingMetrics(
            ...
            batchLoss = totalLoss,
            averageLoss = avgLoss,
            gradientNorm = measuredGradientNorm,
    ...
    fun train(
        ...
        for (epoch in 1..epochs) {
            ...
            var epochLoss = 0.0
            var epochGradientNormSum = 0.0
            var epochGradientSamples = 0
            ...
            while (dataset.hasNext()) {
                ...
                val captureGradient = batchCount == 0
                val batchMetrics = trainBatch(batch, epoch, captureGradient)
                
                epochLoss += batchMetrics.averageLoss
                if (captureGradient) {
                    epochGradientNormSum += batchMetrics.gradientNorm
                    epochGradientSamples++
                }
            ...
            val avgGradientNorm = if (epochGradientSamples > 0) {
                epochGradientNormSum / epochGradientSamples
            } else {
                DEFAULT_GRADIENT_NORM
            }
    ...
    private fun measureGradientNorm(): Double? {
        var sumSquares = 0.0
        var hasData = false
        for (layer in _layers) {
            val layerNorm = layer.gradientL2Norm()
            if (layerNorm != null) {
                sumSquares += layerNorm * layerNorm
                hasData = true
            }
        }
        return if (hasData) sqrt(sumSquares) else null
    }
