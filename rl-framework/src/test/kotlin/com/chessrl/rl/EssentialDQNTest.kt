...
class EssentialDQNTest {
    ...
    fun testNeuralNetworkAgent() {
        ...
        assertTrue(agent.getExplorationRate() >= 0.0, "Exploration should remain non-negative")
    }
    ...
    fun testTargetNetworkUpdate() {
        ...
        val qValuesChanged = initialQValues.keys.any { action ->
            kotlin.math.abs(initialQValues[action]!! - updatedQValues[action]!!) > 1e-8
        }
        assertTrue(qValuesChanged, "Expected target network update to influence action values")
    }
